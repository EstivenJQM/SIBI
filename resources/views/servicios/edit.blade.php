<x-layout title="Editar Servicio">
    ...
    <div class="row justify-content-center">
        <div class="col-md-7">
            <x-card title="<i class='bi bi-pencil-square me-2'></i>Editar Servicio" color="warning">
                <form action="{{ route('servicios.update', $servicio) }}" method="POST">
                        ...
                        <select id="id_linea" name="id_linea" required
                            ...
                            @foreach($lineas as $linea)
                                <option value="{{ $linea->id_linea }}"
                                    {{ old('id_linea', $servicio->id_linea) == $linea->id_linea ? 'selected' : '' }}>
                        ...
                        <select id="id_sede" name="id_sede" required
                            ...
                            @foreach($sedes as $sede)
                                <option value="{{ $sede->id_sede }}"
                                    {{ old('id_sede', $servicio->id_sede) == $sede->id_sede ? 'selected' : '' }}>
                                    {{ $sede->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" id="fecha" name="fecha"
                               ...
                               value="{{ old('fecha', $servicio->fecha->format('Y-m-d')) }}">
                        @error('fecha')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                        ...
                        <select id="id_periodo" name="id_periodo" required
                            ...
                            @foreach($periodos as $periodo)
                                <option value="{{ $periodo->id_periodo }}"
                                    {{ old('id_periodo', $servicio->id_periodo) == $periodo->id_periodo ? 'selected' : '' }}>
                                    {{ $periodo->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <x-form.actions :back="route('servicios.index')" label="Actualizar" />
                </form>
            </x-card>
        </div>
    </div>
    ...
    @php
        ...
        $currentTipo  = old('id_tipo_actividad', $servicio->id_tipo_actividad);
        $currentLinea = old('id_linea', $servicio->id_linea);
    @endphp

    <script>
        const lineasMap = @json($lineasMap);
        const currentLinea = "{{ $currentLinea }}";
        ...
        // Cargar los tipos de la línea actual al abrir el formulario
        if (currentLinea) {
            actualizarTipos(currentLinea, currentTipo);
        }
    </script>
